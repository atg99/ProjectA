	...
	if (ATGPlayerState)
	{
		...
		if (AC)
		{
			...
			if (InvenComp)
			{
                ...
                NET_LOG(FString::Printf(TEXT("Token : %s"), *ATGPlayerState->BackendToken));
			}
		}
	}
...
void AATGGameModeBase::ProcessValidationResult(APlayerController* PC, const FBackendValidateResult& Data)
{
    if (Data.code == 200)
    {
        // 성공: PlayerState에 ID 
        if (AATGPlayerState* PS = PC->GetPlayerState<AATGPlayerState>())
        {
            ...
            UE_LOG(LogTemp, Log, TEXT("Auth Success for: %d, %s %s"), PS->BackendUserID, *PS->BackendUserName, *Data.message);
        }
    }
    else
    {
