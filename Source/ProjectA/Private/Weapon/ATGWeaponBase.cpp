...
void AATGWeaponBase::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_CONDITION(AATGWeaponBase, WeaponData, COND_InitialOnly);
}
...
void AATGWeaponBase::OnRep_WeaponData()
{
...
void AATGWeaponBase::TryHitFire(const FBulletHitResult& BulletHitResult)
{
	NET_LOG(TEXT(""));
...
bool AATGWeaponBase::ServerHitFire_Validate(const FBulletHitResult& BulletHitResult)
{
	NET_LOG(TEXT(""));
...
void AATGWeaponBase::ServerHitFire_Implementation(const FBulletHitResult& BulletHitResult)
{
	NET_LOG(TEXT(""));
...
float AATGWeaponBase::ApplayGunDamage(AActor* DamagedActor, float BaseDamage, float ImpulseScale, FVector const& HitFromDirection, FHitResult const& HitInfo, AController* EventInstigator, AActor* DamageCauser, TSubclassOf<UDamageType> DamageTypeClass)
{
